...
# %%
if RUN:
    EXP_DIR = "/home/pragna/work/chris_ns3/ns-3-dev-git/scratch/traces/DCTCP-final-sim"
    EXP = path.basename(EXP_DIR)
    GRAPH_DIR = path.join("/home/pragna/work/DC_bursts/Analysis-scripts/imc2024-analysis/GRAPH_DIR", "graphs")
...
if RUN:
    ...
    ideal_sec = CONFIG["bytesPerBurstSender"] * CONFIG["maxIncastScale"] / (
...
if RUN:
    MARKING_THRESHOLD = CONFIG["queueMinThresholdPackets"]
    QUEUE_CAPACITY = CONFIG["queueSizePackets"]
...
def parse_flow_times(flow_times_json):
    ...
    for sender in burst_to_sender_to_flow_times[-1].keys():
        sender_flow_times_by_burst = []
        for burst_idx in range(len(burst_to_sender_to_flow_times)):
            if sender in burst_to_sender_to_flow_times[burst_idx]:
...
# %%
len(SENDER_TO_FLOW_TIMES_BY_BURST)
